...
float stdev(float array[], int length){
  float sum2 = 0;
  float mu = mean(array,length);
  for(int i = 0; i < length; i++){
    float difference = mu - array[i];
    sum2 += difference * difference;
  }
  return sqrt(sum2 / length);
}
...
vector<int> removeDups(vector<int> array){
  ...
  for(int i = 0; i < array.size(); i++){
    // Have we already inserted this element?
    int number = array[i];
    bool found = false;
    for(int j = 0; j < ret.size(); j++){
      if(ret[j] == number)found = true;
    }
    if(! found) ret.push_back(number);
  }
  return ret;
}
